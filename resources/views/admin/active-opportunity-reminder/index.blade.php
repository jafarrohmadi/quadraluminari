            ...
            <table class="table table-striped table-bordered datatable-User">
                ...
                @foreach($query as $key => $data)
                    <tr>
                        ...
                        <td>{{ date('Y-m-d', strtotime($data->activeOpportunityHistoryReminderData->last()->act_history_date_reminder)) ?? '' }}</td>
                        <td>{{$data->activeOpportunityHistoryReminderData->last()->act_history_order_reminder ?? '' ?? '' }}</td>
                        <td>
                            @can('active_opportunity_view')
                                <a class="btn btn-xs btn-primary"
                                   href="{{ route('admin.active-opportunity-reminder.show', $data->id) }}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan

                            @can('active_opportunity_edit')
                                <a class="btn btn-xs btn-info" href="{{ route('admin.active-opportunity-reminder.edit', $data->id) }}">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan

                            @can('active_opportunity_delete')
                                <form action="{{ route('admin.active-opportunity-reminder.destroy', $data->id) }}" method="POST"
